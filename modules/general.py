...
def trimmomatic_input(datasets_tab=None, sample=None, library=None, infolder="data/reads"):
    """Input is supposed to be
    
    data/reads/{sample}_{library}.R1.fastq.gz"""
    # print("infolder is {}".format(infolder))
    outpaths = []
    for i, l in datasets_tab.iterrows():
        if l["sample"] == sample and str(l["library"]) == str(library):
            #print(l)
            if is_compr_file(os.path.join(infolder, l["R1"])):
                outpaths.append(os.path.join(infolder, "{sample}_{library}.R1.fastq.gz".format(sample=l["sample"], library=l["library"])))
                outpaths.append(os.path.join(infolder, "{sample}_{library}.R2.fastq.gz".format(sample=l["sample"], library=l["library"])))
                ...
                outpaths.append(os.path.join(infolder, "{sample}_{library}.R1.fastq".format(sample=l["sample"], library=l["library"])))
                outpaths.append(os.path.join(infolder, "{sample}_{library}.R2.fastq".format(sample=l["sample"], library=l["library"])))
    return outpaths
